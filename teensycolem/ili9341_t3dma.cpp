...
static void setDmaStruct() {
  ...
  nbTransfer = SCREEN_DMA_NUM_SETTINGS;   
#endif  
}
...
void ILI9341_t3DMA::begin(void) {
  ...
  dmatx.triggerAtHardwareEvent( DMAMUX_SOURCE_LPSPI4_TX );  // start
#else   
  dmatx.attachInterrupt(dmaInterrupt);
...
#else
  uint8_t *src=buf;
  uint16_t *dst = &screen[ILI9341_TFTWIDTH*y];
  for (int i=0; i<width; i++)
  {
    uint8_t val = *src++;
    *dst++=palette16[val];
